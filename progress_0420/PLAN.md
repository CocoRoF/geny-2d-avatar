...
- **β 로드맵**: 🟢 **P1 S1~S11 완료 + P2 🟢 S1~S7 완료 + P4 🟢 S1 완료** (2026-04-24) — P4-S1 에서 `prompt-slot-planner.ts` 순수 모듈로 1 프롬프트 → N 슬롯 벤더 호출 분해 로직 고정. `extractPromptHints` (영/한 bilingual — "blue hair"/"파란 머리" 양방향 · color/style/skin tone 키워드 카탈로그) + `buildSlotPrompt` (카테고리별 structured 힌트 주입 — Face:eye+skin · Hair:hair · Body:outfit · Accessory:base+tags, **사용자 원본 프롬프트 echo 금지** 로 카테고리 간 힌트 누출 차단) + `planSlotGenerations` (role→Category 분류 + 카테고리당 1 벤더 call 그룹화) + 37 node:test 회귀 (영/한 패턴 · 카테고리 격리 · halfbody 23 파츠 샘플). 37/37 녹색. P2-S7 Generate 재시도/실패 분류 37 node:test. P1-S11 expression ticker alpha ramp 순수 함수 추출 + 17 node:test (경합 구조 제거). P1-S10 motion ticker breath ramp 22 node:test. P2-S6 `?debug=metrics` dev 패널 9 node:test. P1-S9 `?debug=pivots` dev 오버레이. P1-S8 halfbody/fullbody 68 파츠 pivot_uv 자동 주입. P2-S5 metrics emit 스키마 14 node:test. P1-S7 atlas `pivot_uv` 계약. P2-S4 텔레메트리 훅. P1-S6 auto-preview. P2-S3 pill timing. P2-S2 mock 품질. P1-S5 시각 정확성. P1-S4 per-part parameter binding. P0 Q1~Q6 사용자 승인 대기 (비차단).
- **자율 모드**: 🟢 β 범위 활성. SOAK/speculative doc 는 금지 (사용자 2026-04-21 correction).
- **다음 step**: P2-S8 Generate 취소(AbortController) + cleanup. P3 은 `BL-VENDOR-KEY` 블로커 대기 (단 P4-S1 pure function 은 P3 이전 고정 완료 — 벤더 합류 시 본 모듈 불변).
...
| **P2** 프롬프트 UI + Mock e2e | 🟢 **S1+S2+S3+S4+S5+S6+S7 완료** (Generate UI + Mock 생성기 + live swap + 역할별 shape 렌더링 + per-phase timing + β §7 5000ms 예산 시각화 + 구조화 텔레메트리 emit + metric 스키마 14 단위테스트 고정 + `?debug=metrics` dev 패널 9 node:test + Generate 재시도 7-kind 분류 + backoff 플랜 + budget-aware shouldRetry 37 node:test) | 2~3 | P1 완료 | Mock 벤더로 프롬프트→프리뷰 5초 내 완결 **— 측정 기반 완결 + P5 log scraper 대비** |
| **P3** 실 nano-banana 통합 | ⚪ 대기 | 3~5 | P2 완료 + BL-VENDOR-KEY | 실 HTTP 호출 10회 중 7회 이상 성공 |
| **P4** 5 슬롯 자동 조립 | 🟢 **S1 완료** (prompt-slot-planner 순수 모듈: `extractPromptHints` 영/한 bilingual + `buildSlotPrompt` 카테고리 격리 + `planSlotGenerations` end-to-end + `mapRoleToCategory` · 37 node:test 회귀. 벤더 wiring 은 P3 대기, 본 모듈은 합류 시에도 불변) | 3~5 | S1 선행 완료 · 실 wiring 은 P3 완료 후 | 프롬프트 1 줄 → 30초 내 5 슬롯 생성 + atlas |
