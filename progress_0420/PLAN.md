...
| **P4** 5 슬롯 자동 조립 | 🟢 **S1+S2+S3+S4+S5 완료** (prompt-slot-planner 순수 모듈 37 node:test + Mock wire-through — runGenerate 와 auto-preview 모두 `planSlotGenerations` → `mockGenerateTextureFromPlans` 카테고리별 vendor call 시뮬레이션 + per-category status UI + cancel checkpoint 훅 + **P4-S3 per-category metric 축** (`generate.category` 이벤트 + `categoryAverages/Counts/OkCounts` 집계 + dev 패널 cats 행 + 10 새 node:test) + **P4-S4 partial failure 격리** (`category-outcome.ts` 순수 4-status + `mockGenerateTextureFromPlans` try/catch + fault injection 훅 + partial UX ⚠ `부분 성공` + metric ok=false 분리 + 13 새 node:test) + **P4-S5 selective retry** (`partial-retry.ts` 순수 모듈 `selectPlansForRetry` + `mergeCategoryOutcomes` + `CategoryOutcome.retried` 옵션 + 16 새 node:test, Mock `{canvas, retryAttempt}` 옵션 · fault injection `{firstOnly, always}` 확장 · runGenerateAttempt partial→selective retry 1회 · `categoryMetrics` in-place 교체 · UX 3 분기 `✓`/`🔄 재시도 복구`/`⚠ 부분 성공` · metric stopReason 3 분기 success/partial_recovered/partial_success · auto-preview 동일). 벤더 wiring 은 P3 대기, 본 모듈은 합류 시에도 불변) | 3~5 | S1·S2·S3·S4·S5 선행 완료 · 실 wiring 은 P3 완료 후 | 프롬프트 1 줄 → 30초 내 5 슬롯 생성 + atlas |
