...
- **β 로드맵**: 🟢 **P1 S1+S2+S3+S4 완료 + P2 🟡 S1 완료** (2026-04-21) — P1-S4 에서 RendererPart.parameter_ids 계약 확장 + pixi 역색인 (paramToSlots) + 이름 휴리스틱 (angle/sway/offset_x) 기반 per-part transform (rotation/offsetX/offsetY) + 바인드 파츠 없으면 root rotation fallback 유지. MockApp setPartTransformCalls 로 5 회귀 테스트. 30/30 pass. P1 실질 완료: sprite·slider·motion·expression·per-part binding 전부 실 픽셀 반영. P0 Q1~Q6 사용자 승인 여전히 대기 (비차단).
- **자율 모드**: 🟢 β 범위 활성. SOAK/speculative doc 는 금지 (사용자 2026-04-21 correction).
- **다음 step**: P1-S5 (샘플 manifest parameter_ids 주입 + atlas.slots 실 UV), P2-S2 (Mock 품질 개선 — 얼굴 구성요소), 또는 P2-S3 (pill timing 측정). P3 은 `BL-VENDOR-KEY` 블로커 대기.
...
| **P1** 실 픽셀 렌더 | 🟢 **S1+S2+S3+S4 완료** (sprite + atlas slot + slider + motion/expression + per-part binding) | 3~5 | ✅ ADR 0007 Option E Accepted (2026-04-21 P1-S1) | 브라우저에서 aria 실제 픽셀 + slider 변형 실반영 |
