...
| **P4** 5 슬롯 자동 조립 | 🟢 **S1+S2 완료** (prompt-slot-planner 순수 모듈 37 node:test + Mock wire-through — runGenerate 와 auto-preview 모두 `planSlotGenerations` → `mockGenerateTextureFromPlans` 카테고리별 vendor call 시뮬레이션 + per-category status UI + cancel checkpoint 훅. 벤더 wiring 은 P3 대기, 본 모듈은 합류 시에도 불변) | 3~5 | S1·S2 선행 완료 · 실 wiring 은 P3 완료 후 | 프롬프트 1 줄 → 30초 내 5 슬롯 생성 + atlas |
