...
## 1. 현재 상태 (2026-04-22, P1-S9 직후 — **β Phase P1 🟢 S1~S9 완료 · P2 🟢 S1~S5 완료**)

| 축 | 상태 | 비고 |
|---|---|---|
| **단계** | **β Phase P1 🟢 (S1~S9 완료) + P2 🟢 (S1~S5 완료)** (Foundation ✅ 종료, P0 UX wireframe 산출물 완료 · Q1~Q6 사용자 승인은 비차단 대기) | P1-S9 (2026-04-22) — **`?debug=pivots` dev 오버레이**. pixi 렌더러 `computePivotMarkerPositions` 순수 함수 + `defaultCreateApp` 조건부 Graphics 마커 레이어(빨강=저자 주입, 회색=slot-center fallback) + web-editor `?debug=pivots` URL flag 배선. P1-S8 이 주입한 halfbody/fullbody 68 파츠 pivot_uv 의 실 화면 위치를 육안 검증 가능 + AI 생성 variant 에서 fallback 슬롯이 가시적. pixi 44/44 (신규 8 단위 — fit/origin 공식 + halfbody ahoge 실값 회귀). 기본 경로 zero-cost (pixi.Container 조건부). P1-S8 — halfbody/fullbody 68 파츠 pivot_uv 자동 주입 + P1-S7 uv 포맷 회귀 수정. P2-S5 — metrics emit 스키마 회귀 고정 (`buildGenerateMetricEvents()` + 14 node:test). P1-S7 — atlas `pivot_uv` optional 계약. P2-S4 텔레메트리 훅. P1-S6 auto-preview 로 4×4 placeholder 제거. P2-S3 — regenerate Promise<void> + per-phase ms + β §7 예산 시각화. P2-S2 — 16 역할 카테고리. P1-S5 — sprite.anchor=0.5 + Cubism 3 축 분리. P1-S4 — parts 별 parameter 바인딩. P1-S3 — motion/expression. P1-S2+P2-S1 — atlas UV + Generate bar. P1-S1 — ADR 0007 Option E Accepted + pixi scaffold. |
| **Foundation Exit 게이트** | **4/4 ✅** | E2E / CI 골든 / 관측 / 온보딩 — 모두 자동 회귀 |
| **릴리스 게이트 (보안/성능/온콜)** | **3/3 ✅** | docs/14 §10 |
| **누적 세션** | 128 Foundation + P0-S1 + P1-S1 + P1-S2+P2-S1 + P1-S3 + P1-S4 + P1-S5 + P2-S2 + P2-S3 + P1-S6 + P2-S4 + P1-S7 + P2-S5 + P1-S8 + P1-S9 (2026-04-17~04-22, 6일) | Foundation 연대기 1~127 동결. 128 β 모드 전환 이후는 phase+step ID (`P0-S1` / `P1-S1` / `P1-S2+P2-S1` / `P1-S3` / `P1-S4` / `P1-S5` / `P2-S2` / `P2-S3` / `P1-S6` / `P2-S4` / `P1-S7` / `P2-S5` / `P1-S8` / `P1-S9`...). |
