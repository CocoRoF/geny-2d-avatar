...
## 1. 현재 상태 (2026-04-21, P1-S3 직후 — **β Phase P1 🟢 실질 완료 · P2 🟡 S1 완료**)

| 축 | 상태 | 비고 |
|---|---|---|
| **단계** | **β Phase P1 🟢 (S1+S2+S3 완료) + P2 🟡 (S1 완료)** (Foundation ✅ 종료, P0 UX wireframe 산출물 완료 · Q1~Q6 사용자 승인은 비차단 대기) | P1-S3 (2026-04-21) — **motion/expression → pixi 바인딩 개통**. `@geny/web-avatar-renderer` contract 에 `RendererMotion`/`RendererExpression`/`RendererMotionStartEventDetail`/`RendererExpressionChangeEventDetail` 4 타입 추가. pixi 렌더러가 `motionstart`/`expressionchange` 구독 + `PixiAppHandle.setMotion(motion|null)` / `setExpression(expression|null)` 호출 + `lastMotion`/`lastExpression`/`motionStartCount`/`expressionChangeCount` 공개. `defaultCreateApp` 에 idle breath ticker (loop=true motion 시 root.scale.y = 1 + sin(2π·t/duration_sec)·0.04·rampFactor, fade_in/fade_out 선형 램프) + expression alpha blink. createApp 완료 전 이벤트 catch-up replay. 25/25 pixi test pass (+5). P1-S2+P2-S1 (2026-04-21) — `deriveAtlasFromTemplate` 가 PartSpec canvas_px+uv_box_px 에서 halfbody 30 / fullbody 38 slot UV 를 정규화 파생 (v1.2.0 goldens 보존 위해 `atlasOverride` opt-in). pixi 렌더러가 `pixi.Assets.load`+`new PIXI.Texture({ source, frame })` 로 실 sprite 렌더. `PixiRenderer.regenerate({ atlas, textureUrl })` 로 ready 재-dispatch 없이 live swap. web-editor Generate bar + 5-pill + 브라우저-로컬 Mock 생성기 (hash(prompt+slot_id) HSL radial-gradient + blob URL). P1-S1 (2026-04-21) — ADR 0007 **Option E Accepted** + `packages/web-avatar-renderer-pixi/` scaffold (PixiJS v8 MIT). |
| **Foundation Exit 게이트** | **4/4 ✅** | E2E / CI 골든 / 관측 / 온보딩 — 모두 자동 회귀 |
| **릴리스 게이트 (보안/성능/온콜)** | **3/3 ✅** | docs/14 §10 |
| **누적 세션** | 128 Foundation + P0-S1 + P1-S1 + P1-S2+P2-S1 + P1-S3 (2026-04-17~04-21, 5일) | Foundation 연대기 1~127 동결. 128 β 모드 전환 이후는 phase+step ID (`P0-S1` / `P1-S1` / `P1-S2+P2-S1` / `P1-S3`...). |
| **누적 패키지** | **15** packages + 3 apps + 1 service | TypeScript ESM, pnpm workspace. P1-S1 에서 `@geny/web-avatar-renderer-pixi` 합류 (14 → 15). P1-S2~S3 에선 기존 2 패키지 확장만 (renderer contract atlas+motion+expression 필드 + renderer-pixi sprite/regenerate/motion/expression). |
