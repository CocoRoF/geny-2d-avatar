...
## 1. 현재 상태 (2026-04-22, P1-S8 직후 — **β Phase P1 🟢 S1~S8 완료 · P2 🟢 S1~S5 완료**)

| 축 | 상태 | 비고 |
|---|---|---|
| **단계** | **β Phase P1 🟢 (S1~S8 완료) + P2 🟢 (S1~S5 완료)** (Foundation ✅ 종료, P0 UX wireframe 산출물 완료 · Q1~Q6 사용자 승인은 비차단 대기) | P1-S8 (2026-04-22) — **halfbody v1.3.0 30 파츠 + fullbody v1.0.0 38 파츠 전 파츠 `atlas.slots[i].pivot_uv` 자동 주입** + P1-S7 `resolvePivotPlacement` uv 포맷(`[x,y,w,h]` vs `[u0,v0,u1,v1]`) 회귀 수정. exporter `deriveSlotsFromSpecs` 가 part spec `anchor.x_frac/y_frac` (slot-local) 을 `(box.x + x_frac·box.w)/W` 공식으로 캔버스 UV 환산, `apps/web-editor/scripts/prepare.mjs` 의 `deriveAtlasFromTemplate` 경로로 번들 자동 반영 — ahoge `[0.5, 0.021484375]` 등 실 peg 가 렌더러 `resolvePivotPlacement` 로 흐른다. exporter-core 107/107 (신규 단위 1 + 골든 2 확장) + pixi 36/36 (3 테스트 [x,y,w,h] 컨벤션 재작성) + pipeline 10/10 (v1.2.0 sha256 영향 없음 — `runWebAvatarPipeline` 은 derive 미사용) + web-editor e2e halfbody/fullbody 전 green. P2-S5 — metrics emit 스키마 회귀 고정 (`buildGenerateMetricEvents()` + 14 node:test). P1-S7 — atlas `pivot_uv` optional 계약. P2-S4 텔레메트리 훅. P1-S6 auto-preview 로 4×4 placeholder 제거. P2-S3 — regenerate Promise<void> + per-phase ms + β §7 예산 시각화. P2-S2 — 16 역할 카테고리. P1-S5 — sprite.anchor=0.5 + Cubism 3 축 분리. P1-S4 — parts 별 parameter 바인딩. P1-S3 — motion/expression. P1-S2+P2-S1 — atlas UV + Generate bar. P1-S1 — ADR 0007 Option E Accepted + pixi scaffold. |
| **Foundation Exit 게이트** | **4/4 ✅** | E2E / CI 골든 / 관측 / 온보딩 — 모두 자동 회귀 |
| **릴리스 게이트 (보안/성능/온콜)** | **3/3 ✅** | docs/14 §10 |
| **누적 세션** | 128 Foundation + P0-S1 + P1-S1 + P1-S2+P2-S1 + P1-S3 + P1-S4 + P1-S5 + P2-S2 + P2-S3 + P1-S6 + P2-S4 + P1-S7 + P2-S5 + P1-S8 (2026-04-17~04-22, 6일) | Foundation 연대기 1~127 동결. 128 β 모드 전환 이후는 phase+step ID (`P0-S1` / `P1-S1` / `P1-S2+P2-S1` / `P1-S3` / `P1-S4` / `P1-S5` / `P2-S2` / `P2-S3` / `P1-S6` / `P2-S4` / `P1-S7` / `P2-S5` / `P1-S8`...). |
