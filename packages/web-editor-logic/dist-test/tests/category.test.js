...
import { CATEGORY_ORDER, categorize, categoryOf, } from "../src/index.js";
const HALFBODY_ROLES = [
...
describe("categorize — group + sort", () => {
    ...
    test("within a category parts are sorted by slot_id", () => {
        const groups = categorize(parts);
        assert.deepEqual(groups.get("Hair")?.map((p) => p.slot_id), ["hair_back", "hair_front"]);
        assert.deepEqual(groups.get("Body")?.map((p) => p.slot_id), ["arm_l", "arm_r"]);
    });
...
describe("세션 87 — 실 rig-templates 샘플 총 카디널리티 회귀", () => {
    test("halfbody 29 parts → Face=16/Hair=4/Body=7/Accessory=2", () => {
        ...
        const parts = roles.map((r, i) => ({ role: r, slot_id: `s_${i}` }));
