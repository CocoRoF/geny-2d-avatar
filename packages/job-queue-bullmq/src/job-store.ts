 ...
 *    없으면 `undefined`.
 *
 *  - **orchestrate 경로는 JobStore 쪽에서 실행**: driver 가 BullMQ 를 "단순 queue" 로 쓰는 건
 *    의도적 축약. Runtime N+1 (X+1 세션) 에서 **실** BullMQ 바인딩이 생기면 `Worker` 가 별
 *    프로세스에서 consumer 를 돌리지만, X 단계는 submit → orchestrate 를 같은 프로세스에서
 *    수행해 계약 테스트에 집중 (요청 경로 코드 총량 최소화). Worker 분리는 X+1.
 *
...
export function createBullMQJobStore(opts: CreateBullMQJobStoreOptions): BullMQJobStore {
  ...
  return {
    async submit(task) {
      ...
      // 동일 idempotency_key 재제출: 기존 레코드 그대로 반환.
      ...
      // orchestrate 를 백그라운드 실행 (submit 은 `queued` 상태로 반환). 실 BullMQ 바인딩(X+1)
      // 에서는 별 프로세스 Worker 가 돌리므로 이 훅이 사라짐. `setImmediate` 로 다음 매크로
      // 태스크까지 지연 — `await submit(...)` 마이크로태스크 체인이 먼저 완료되어야 호출자가
      // `status === "queued"` 를 관측할 수 있다.
