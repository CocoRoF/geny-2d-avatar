...
test("flux-fill: rejects edit/style_ref — CAPABILITY_MISMATCH", async () => {
  ...
  await assert.rejects(
    a.generate(makeTask("face_base", ["edit"])),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
  await assert.rejects(
    a.generate(makeTask("face_base", ["style_ref"])),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
});
...
test("flux-fill: missing reference_image → CAPABILITY_MISMATCH", async () => {
  ...
  await assert.rejects(
    a.generate(
      makeTask("face_base", ["mask"], { reference_image_sha256: null }),
    ),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
});
...
test("flux-fill: missing mask → CAPABILITY_MISMATCH", async () => {
  const a = new FluxFillAdapter();
  await assert.rejects(
    a.generate(makeTask("face_base", ["mask"], { mask_sha256: null })),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
});
...
test("flux-fill: BUDGET_EXCEEDED when estimate > budget", async () => {
  const a = new FluxFillAdapter();
  await assert.rejects(
    a.generate(makeTask("face_base", ["mask"], { budget_usd: 0.001 })),
