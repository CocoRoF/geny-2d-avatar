...
test("sdxl: mask capability is rejected — CAPABILITY_MISMATCH", async () => {
  ...
  await assert.rejects(
    a.generate(makeTask("face_base", ["mask"], { mask_sha256: "b".repeat(64) })),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
});
...
test("sdxl: upscale capability rejected", async () => {
  const a = new SDXLAdapter();
  await assert.rejects(
    a.generate(makeTask("hair_front", ["upscale"])),
    (e: unknown) => (e as { code?: string }).code === "CAPABILITY_MISMATCH",
  );
});
...
test("sdxl: BUDGET_EXCEEDED when estimate exceeds budget", async () => {
  const a = new SDXLAdapter();
  await assert.rejects(
    a.generate(makeTask("hair_front", ["edit"], { budget_usd: 0.001 })),
...
test("sdxl: INVALID_OUTPUT if client returns bad sha", async () => {
  ...
  await assert.rejects(
    a.generate(makeTask("hair_front", ["edit"])),
    (e: unknown) => (e as { code?: string }).code === "INVALID_OUTPUT",
  );
});
