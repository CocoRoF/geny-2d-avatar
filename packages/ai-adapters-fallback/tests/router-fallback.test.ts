...
test("router: style_ref → [nano-banana, sdxl]", () => {
  const r = buildRegistry();
  const candidates = r.route(
    makeTask("outfit_upper", ["style_ref"], {
      style_reference_sha256: ["a".repeat(64)],
    }),
  );
  assert.deepEqual(
    candidates.map((a) => a.meta.name),
    ["nano-banana", "sdxl"],
  );
});
...
test("router: fallback — nano-banana 실패 시 sdxl 으로 내려 edit 처리", async () => {
  ...
  assert.ok(primary);
  const mocked = Object.create(primary) as typeof primary & {
    generate: (t: GenerationTask) => Promise<never>;
  };
  mocked.generate = async () => {
    throw new Error("simulated nano-banana 503");
  };
  let lastError: unknown = null;
  let result: { vendor: string } | null = null;
...
test("router: unsupported capability → NO_ELIGIBLE_ADAPTER", () => {
  const r = buildRegistry();
  assert.throws(
    () => r.route(makeTask("some_slot", ["upscale"])),
    (e: unknown) => (e as { code?: string }).code === "NO_ELIGIBLE_ADAPTER",
  );
});
...
test("router: budget below all candidates → NO_ELIGIBLE_ADAPTER", () => {
  const r = buildRegistry();
  assert.throws(
    () => r.route(makeTask("hair_front", ["edit"], { budget_usd: 0.0001 })),
    (e: unknown) => (e as { code?: string }).code === "NO_ELIGIBLE_ADAPTER",
  );
});
...
test("router: sort is deterministic — weight desc, cost asc, name asc", () => {
  ...
  assert.deepEqual(
    first.map((a) => a.meta.name),
    second.map((a) => a.meta.name),
  );
});
