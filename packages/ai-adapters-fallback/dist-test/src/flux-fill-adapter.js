...
const FLUX_FILL_CAPABILITIES = ["mask"];
export class FluxFillMockClient {
    modelVersion = "flux-fill-1.0-mock";
    ...
    async generate(task) {
        for (const req of task.capability_required ?? []) {
            if (!FLUX_FILL_CAPABILITIES.includes(req)) {
                throw new AdapterError(`flux-fill does not support capability '${req}'`, "CAPABILITY_MISMATCH", { task_id: task.task_id, required: req });
            }
        }
        if (!task.reference_image_sha256 || !task.mask_sha256) {
            throw new AdapterError("flux-fill requires reference_image_sha256 and mask_sha256", "CAPABILITY_MISMATCH", {
        ...
        if (!/^[0-9a-f]{64}$/.test(response.image_sha256)) {
            throw new AdapterError("flux-fill vendor returned invalid image_sha256", "INVALID_OUTPUT", { task_id: task.task_id });
        }
        return {
            schema_version: "v1",
            task_id: task.task_id,
...
//# sourceMappingURL=flux-fill-adapter.js.map