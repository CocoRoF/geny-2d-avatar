...
// atlas.json 의 slot uv 는 [x, y, w, h] (정규화, 좌상단 + 폭/높이). P1-S8 에서
// resolvePivotPlacement 를 이 실 포맷에 맞춰 교정.
test("resolvePivotPlacement: pivot_uv 없으면 slot 중심 (이전 anchor=0.5 동작) (P1-S7/S8)", () => {
  // uv = [x=0.2, y=0.1, w=0.6, h=0.5] → 중심 = (0.5, 0.35).
  const res = resolvePivotPlacement({
    slot: { uv: [0.2, 0.1, 0.6, 0.5] },
    frame: { x: 204, y: 102, width: 614, height: 512 },
    canvasW: 1024,
    canvasH: 1024,
    fit: 1,
    originX: 0,
  ...
  assert.ok(Math.abs(res.spriteX - 0.5 * 1024) < 1e-9);
  assert.ok(Math.abs(res.spriteY - 0.35 * 1024) < 1e-9);
});

test("resolvePivotPlacement: pivot_uv = 슬롯 우상단 → anchor 우상단, position = 우상단 (P1-S7/S8)", () => {
  // slot = [x=0.2, y=0.1, w=0.6, h=0.5], 우상단 UV = (x+w, y) = (0.8, 0.1).
  const res = resolvePivotPlacement({
    slot: { uv: [0.2, 0.1, 0.6, 0.5], pivot_uv: [0.8, 0.1] },
    frame: { x: 204, y: 102, width: 614, height: 512 },
    canvasW: 1024,
    canvasH: 1024,
    fit: 1,
  ...
  // anchor = ((0.8-0.2)/0.6, (0.1-0.1)/0.5) = (1, 0) = sprite 우상단.
  assert.ok(Math.abs(res.anchorX - 1) < 1e-9);
  assert.ok(Math.abs(res.anchorY - 0) < 1e-9);
  assert.ok(Math.abs(res.spriteX - 0.8 * 1024) < 1e-9);
  assert.ok(Math.abs(res.spriteY - 0.1 * 1024) < 1e-9);
});

test("resolvePivotPlacement: pivot_uv 가 슬롯 바깥 (머리 위) 이어도 안전 + 절대 좌표 정확 (P1-S7/S8)", () => {
  // ahoge (더듬이) 는 얼굴 위쪽이 피벗. slot = [0.4, 0, 0.2, 0.1] (x,y,w,h).
  // pivot_uv = [0.5, 0.2] — y=0.2 가 slot 바깥(slot 은 y=[0, 0.1]).
  const res = resolvePivotPlacement({
    slot: { uv: [0.4, 0.0, 0.2, 0.1], pivot_uv: [0.5, 0.2] },
  ...
  // anchorX = (0.5-0.4)/0.2 = 0.5, anchorY = (0.2-0)/0.1 = 2 (외삽)
  assert.ok(Math.abs(res.anchorX - 0.5) < 1e-9);
  assert.ok(Math.abs(res.anchorY - 2) < 1e-9);
