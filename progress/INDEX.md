...
| **AI Generation** | nano-banana 어댑터 | 🟡 계약 + 1차 skeleton + 폴백 skeleton (세션 22/25) — `@geny/ai-adapter-core` v0.1.0 (AdapterRegistry + 결정론적 시드 + provenance 빌더 + `buildCacheKey`/`InMemoryAdapterCache`, 23 tests) + `@geny/ai-adapter-nano-banana` v0.1.0 (Mock + adapter→provenance→license-verifier round-trip + `HttpNanoBananaClient` Gemini 2.5 Flash Image HTTP 클라이언트, 벤더 에러 → `AdapterError` 매핑, 23 tests) + `@geny/ai-adapters-fallback` v0.1.0 (SDXL `edit/style_ref` routing_weight=80 + Flux-Fill `mask` routing_weight=70 Mock skeleton + `AdapterRegistry` 통합 폴백 순서 회귀, 31 tests). HttpSDXL/HttpFluxFill + `routeWithFallback` 헬퍼는 세션 26+ |
...
| **UX** | 에디터 뼈대 | ⚪ 미착수 |
| **Platform / Infra** | K8s + CI/CD + 관측 | 🟡 CI (세션 10/13b/20/21/22/23/24/25/26/27 — `test:golden` 14 step: schemas + exporter-core + bundle/avatar/web-avatar golden + web-preview e2e + license-verifier + ai-adapter-core + ai-adapter-nano-banana + web-avatar dom lifecycle + observability chart verify + ai-adapters-fallback + post-processing + rig-template migrate) + 관측 Helm chart (세션 17/24) + 공개키 레지스트리 (세션 21) |
...
- [x] 골든셋 회귀 통과 — `@geny/exporter-core` 14 fixture (halfbody Cubism 11 + aria 번들 1 + halfbody web-avatar 2) + 번들 루트 `bundle.json` 해시 감사 (세션 13) + `pnpm run test:golden` 14 step CI (세션 08/08b/09/10/11/12/13/15/20/21/22/23/24/25/26/27 — step 6 = web-preview e2e, step 7 = license-verifier, step 8 = ai-adapter-core (23), step 9 = ai-adapter-nano-banana +HttpNanoBananaClient (23), step 10 = web-avatar dom lifecycle, step 11 = observability chart verify, step 12 = ai-adapters-fallback (31), step 13 = post-processing (27), step 14 = rig-template migrate (v1.0.0→v1.3.0 체인 + 결정론))
...
- **세션 27**: rig 확장 (v1.3 body) — 하체 파츠 스펙 + 물리 3 Setting 추가(`body_breath_phys` + 잔여 `ahoge`/`accessory`) + migration `v1.2.0 → v1.3.0`.
- **세션 28**: AI 어댑터 3차 — `routeWithFallback(registry, task, { maxAttempts, cache })` 헬퍼 (5xx 시 다음 후보, cache hit 시 즉시 반환) + HttpSDXL/HttpFluxFill HTTP 클라이언트 + 안전 필터 훅(docs/05 §9).
