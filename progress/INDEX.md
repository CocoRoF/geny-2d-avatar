...
| **Post-Processing & Fitting** | Stage 1, 3, 6 (alpha/color/pivot) | 🟡 Stage 1 skeleton (세션 26) — `@geny/post-processing` v0.1.0: `premultipliedToStraight`/`straightToPremultiplied` (α=255 bit-exact 라운드트립) + `cleanAlphaNoise` (default threshold 8) + `computeAlphaBbox` (tight bbox) + `applyAlphaSanitation` 파이프라인 + 결과 sha256 골든 회귀. 27 tests. morphological close/feather/uv-clip 은 라이브러리·메타 연결 후 후속 |
| **UX** | 에디터 뼈대 | ⚪ 미착수 |
| **Platform / Infra** | K8s + CI/CD + 관측 | 🟡 CI (세션 10/13b/20/21/22/23/24/25/26/27/28 — `test:golden` 14 step: schemas + exporter-core + bundle/avatar/web-avatar golden + web-preview e2e + license-verifier + ai-adapter-core (+`routeWithFallback`) + ai-adapter-nano-banana + web-avatar dom lifecycle + observability chart verify + ai-adapters-fallback (+HttpSDXL/HttpFluxFill) + post-processing + rig-template migrate) + 관측 Helm chart (세션 17/24) + 공개키 레지스트리 (세션 21) |
...
- [x] 골든셋 회귀 통과 — `@geny/exporter-core` 14 fixture (halfbody Cubism 11 + aria 번들 1 + halfbody web-avatar 2) + 번들 루트 `bundle.json` 해시 감사 (세션 13) + `pnpm run test:golden` 14 step CI (세션 08/08b/09/10/11/12/13/15/20/21/22/23/24/25/26/27/28 — step 6 = web-preview e2e, step 7 = license-verifier, step 8 = ai-adapter-core 37 (+routeWithFallback 14), step 9 = ai-adapter-nano-banana +HttpNanoBananaClient (23), step 10 = web-avatar dom lifecycle, step 11 = observability chart verify, step 12 = ai-adapters-fallback 53 (+HttpSDXL/HttpFluxFill 22), step 13 = post-processing (27), step 14 = rig-template migrate (v1.0.0→v1.3.0 체인 + 결정론))
...
- **세션 29**: Post-Processing Stage 3 (color normalize) skeleton — per-part 색 통계 + 목표 색 재맵. docs/06 §6 의 결정론적 경로만 먼저.
