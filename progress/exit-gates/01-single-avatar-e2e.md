...
- [ ] `pnpm --filter @geny/web-preview run test` 1발 실행 → `✅ web-preview e2e pass`.
  - 내부: prepare → serve(random port) → HTTP 6종 200 + MIME 검증 → `loadWebAvatarBundle` 체인 (manifest/meta/atlas) 검증 → serve 종료. 실패 시 non-zero exit.
- [ ] `pnpm run test:golden` 의 step 6 `web-preview e2e` 도 same (CI 자동 회귀).

**D-시각 (수동, 선택)** — Custom Element 실제 DOM 동작 확인용:
- [ ] `pnpm --filter @geny/web-preview run dev` → `http://localhost:4173/` 방문.
- [ ] **상태** 박스가 `ready — tpl.base.v1.halfbody@1.2.0` (녹색) 로 전환.
- [ ] **Bundle Manifest** 섹션: `kind == web-avatar-bundle`, `avatar_id == avt.preview.halfbody.demo`, `files (count) == 3`.
- [ ] **Web Avatar Meta**: parameters/parts/motions/expressions 모두 >=1, `physics_summary` 표시, `atlas ref` 존재.
- [ ] **Atlas**: `textures` 에 `textures/base.png · 4×4 · png` 1 라인.
...
- Custom Element DOM lifecycle 자동 검증 (happy-dom/Playwright — 세션 20 의 D-자동은 HTTP+loader 축만 커버).
...
Foundation 이후:
1. happy-dom 혹은 Playwright 로 `<geny-avatar>` 실 DOM 라이프사이클까지 검증 (`ready` event 페이로드 스냅샷).
