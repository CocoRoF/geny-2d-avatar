...
- **Date**: 2026-04-21 (세션 113, 자율 모드 작성)
- **Deciders**: geny-core (Decision 공란 — 사용자 리뷰 대기)
...
**공란 (Draft).** 본 ADR 은 사용자 / PM 리뷰를 위한 **옵션 정렬 문서**. 리뷰 이후 다음 중 하나로 확정한다:

1. **Option E (하이브리드) — 권장 기본값**: 세션 N~N+5 범위에서 PixiJS 구현으로 β 렌더러 닫고, GA 전 Spike 세션에서 자체 런타임 시험. 인터페이스는 첫 Spike 부터 별도 패키지로 분리.
2. **Option D (자체 WebGL2) 직진**: β 납기를 뒤로 미루더라도 외부 의존 0 을 달성. Foundation 이후 일정 여유가 있을 때만.
3. **Option A (PixiJS) 고정**: GA 교체 계획 없이 Pixi 로 장기. 번들 크기 / 라이선스 / 런타임 성능이 허용하는 경우.
4. **Option C (Cubism Web SDK) 부분 채택**: "Cubism 네이티브 에셋 Viewer" 가 제품 요구로 확정되면 런타임 렌더러 축에 **별도 레인** 으로 도입. 기본 렌더러는 여전히 A/D/E 중 택.

Option B (Three.js) 는 본 저자의 판단으로는 fit 열위 — 명시적 반대 의견이 없다면 탈락 후보.
...
- [ ] 사용자 / PM 리뷰 → Decision 확정 → 본 ADR Status "Accepted" 로 승격 + 커밋.
- [ ] 결정된 경로의 Spike 세션 예약 (세션 114 후보). Spike 산출물 = 하나의 번들 + 하나의 파라미터 슬라이더 → 회전하는 파츠 한 개.
