...
export function createPollinationsAdapter(opts = {}) {
    ...
    const enabled = opts.enabled ?? process.env.GENY_POLLINATIONS_DISABLED !== "true";
    ...
    return {
        ...
        async generate(task) {
            ...
            try {
                ...
                clearTimeout(t);
                const e = err;
                const code = e.name === "AbortError" || /timeout|timed out/i.test(e.message)
                    ? "TIMEOUT"
                    : "NETWORK_ERROR";
                const wrapped = new Error("pollinations fetch failed (" + code + "): " + e.message);
                wrapped.code = code;
                throw wrapped;
            }
            clearTimeout(t);
            ...
            const isPng = buf.length >= 8 &&
                ...
                buf[7] === 0x0a;
            if (!isPng) {
                const err = new Error("pollinations 반환 이미지가 PNG 아님 (content-type=" +
                    contentType +
                    ", bytes=" +
                    buf.length +
...
//# sourceMappingURL=pollinations-adapter.js.map