...
import { TextureAdapterRegistry, runTextureGenerate, NoEligibleAdapterError, AllAdaptersFailedError, } from "../src/lib/texture-adapter.js";
...
test("runTextureGenerate: eligible 없으면 NoEligibleAdapterError", async () => {
    ...
    await assert.rejects(() => runTextureGenerate(task({ width: 64, height: 64 }), r), NoEligibleAdapterError);
});
...
test("runTextureGenerate: 모든 어댑터 실패 → AllAdaptersFailedError + attempts 제공", async () => {
    ...
    try {
        ...
        assert.equal(err.attempts[0].adapter, "a");
        assert.equal(err.attempts[1].adapter, "b");
    }
});
...
//# sourceMappingURL=texture-adapter.test.js.map