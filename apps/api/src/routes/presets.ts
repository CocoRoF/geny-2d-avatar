...
export interface PresetsRouteOptions {
  /** rig-templates/ 루트 절대 경로. 테스트에서 주입, 실 서버는 환경 변수 또는 기본값 사용. */
...
interface ManifestShape {
  ...
  readonly display_name: { readonly en: string; readonly ko?: string; readonly ja?: string };
  readonly family: string;
  readonly canvas: { readonly width: number; readonly height: number };
  readonly origin?: { readonly kind: "third-party" | "derived" | "user" };
}
...
async function scanPresetDir(presetDir: string) {
  const manifestPath = join(presetDir, "template.manifest.json");
  const manifest = await readJson<ManifestShape>(manifestPath);
  if (!manifest) return null;
  const atlas = await readJson<AtlasShape>(join(presetDir, "textures", "atlas.json"));
 ...
 */
export async function scanPresets(rigTemplatesRoot: string) {
  const baseDir = join(rigTemplatesRoot, "base");
  if (!existsSync(baseDir)) return [];

  const results = [] as Awaited<ReturnType<typeof scanPresetDir>>[];
  ...
  for (const slug of slugs) {
    ...
    for (const v of versions) {
      if (!/^v[0-9]+\.[0-9]+\.[0-9]+$/.test(v)) continue;
...
export const presetsRoute: FastifyPluginAsync<PresetsRouteOptions> = async (fastify, opts) => {
