...
async function readAtlas(rigTemplatesRoot: string, id: string, version: string) {
  ...
  try {
    ...
    return JSON.parse(buf) as AtlasShape;
  } catch {
    return null;
  }
}

export const textureUploadRoute: FastifyPluginAsync<TextureUploadRouteOptions> = async (
  fastify,
  opts,
) => {
  ...
  fastify.post("/api/texture/upload", async (request, reply) => {
    ...
    for await (const part of request.parts()) {
      if (part.type === "field") {
        if (part.fieldname === "preset_id") presetId = String(part.value);
        else if (part.fieldname === "preset_version") presetVersion = String(part.value);
