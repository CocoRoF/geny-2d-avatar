...
function seedTexture(texturesDir: string, textureId: string): void {
...
test("POST /api/build: halfbody v1.3.0 번들 조립 성공", async () => {
  ...
  try {
    ...
    assert.ok(
      existsSync(join(bundleDir, "textures", "test_halfbody_00.png")),
      "uploaded texture copied to bundle",
    );
  } finally {
    await app.close();
...
test("GET /api/bundle/:id/bundle.json: 생성된 번들 파일 조회", async () => {
  ...
  try {
    ...
    const bundleId = (buildRes.json() as { bundle_id: string }).bundle_id;
    ...
    assert.match(bundleJsonRes.headers["content-type"] as string, /application\/json/);
...
test("GET /api/bundle/:id/download: zip 스트리밍", async () => {
  ...
  try {
    ...
    assert.match(
      dl.headers["content-disposition"] as string,
      new RegExp('attachment; filename="' + bundleId + '\\.zip"'),
    );
