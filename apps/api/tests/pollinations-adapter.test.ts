...
function mockFetchOk(buf: Buffer, contentType = "image/png"): typeof fetch {
  return (async () =>
...
function mockFetchStatus(status: number): typeof fetch {
  return (async () => new Response("error", { status })) as unknown as typeof fetch;
}
...
test("pollinations URL 구성 검증 (prompt/width/height/seed/model 파라미터)", async () => {
  let capturedUrl: URL | string | null = null;
  const a = createPollinationsAdapter({
    model: "flux",
