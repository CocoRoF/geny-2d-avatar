  ...
  try {
    ...
    const body = res.json() as {
      presets: Array<{
        ...
        atlas: { width: number; height: number; slot_count: number } | null;
        motion_count: number;
    ...
    assert.ok(
      body.presets.length >= 3,
      "expected at least 3 presets, got " + body.presets.length,
    );
...
test("/api/presets 응답은 id@version 알파벳 정렬", async () => {
  ...
  try {
    const res = await app.inject({ method: "GET", url: "/api/presets" });
    const body = res.json() as { presets: Array<{ id: string; version: string }> };
