# SUMMARY — 세션 1~126 누적 결과 심층 정리

본 문서는 5 일간(2026-04-17~04-21) 126 세션의 누적 결과를 **워크스트림 + 시간순 마일스톤** 으로 재정리한다. 원문 로그는 `progress/sessions/`, ADR은 `progress/adr/`, 탐색 노트는 `progress/notes/`, 운영 런북은 `progress/runbooks/` (01 P1 인시던트 + 02 golden step + 03 rig-template-lint 규칙), 스키마 카탈로그는 `schema/README.md`, 리그 템플릿 실측 카탈로그는 `rig-templates/README.md`. 주장(claims) 축과 세션 로그 축은 분리 — 세션 121~126 의 메타/색인 작업 이후 claims 에서 발견된 드리프트는 본 문서/INDEX/memory/해당 README 에서만 수정, 세션 로그(`progress/sessions/*`) 는 역사 그대로 보존.
...
| rig-template-lint 규칙 카탈로그 | ✅ 완료 (세션 124) | `progress/runbooks/03-rig-template-lint-rules.md` 신규 (~240 줄). `scripts/rig-template/rig-template-lint.mjs` 의 14 규칙 × 34 테스트를 4-라인(보장/실행/의존성/도입) 색인. §1 meta 카운트(C1~C5) / §2 input·output(C6~C9) / §3 네이밍(C10 2-sub) / §4 cross-file 파라미터(C11·C12) / §5 deformer 트리(C13 7-sub) / §6 parts↔deformers(C14) / §7 FAMILY_OUTPUT_RULES 6 family 테이블 / §8 CLI 옵션 / §9 테스트 매핑 / §10 참고. runbook 축 3 종(01 비상 + 02 정상 CI + 03 저자 개입) 완성. runbook README 에 03 entry 추가. doc-only, 코드 변경 0, 골든 영향 0 |
| rig-templates/README 실측 카탈로그 | ✅ 완료 (세션 125) | `rig-templates/README.md` 재작성 — 기존 43 줄 스테일 문서(v1.0.1 예시 · parts 24개 가정 · v1.0.0 구조만 전제 · fullbody 미언급) 를 5 공식 템플릿(halfbody v1.0.0~v1.3.0 + fullbody v1.0.0) × 8 실측 컬럼(family/parts/parameters/deformers/physics_settings/expressions/motions/test_poses) 카탈로그로 교체. `overall_*` 구조 파라미터 해석 차이로 버전별 `v*.*.*/README.md` narrative 가 -1 표기 (halfbody v1.3.0: JSON 50 / narrative 49, fullbody v1.0.0: JSON 60 / narrative 59) — **JSON 실측을 권위로** 삼고 버전 README 는 역사 narrative 로 보존. 진화 축 명시(pose.json v1.1.0+, expressions/ textures/ v1.2.0+). 인접 드리프트 1 건 해소: INDEX §2 Rig & Parts "49+10 파라미터" → "50+10 (JSON 실측)" + parts 분모 명시(halfbody 19/30 · fullbody 27/38). 검증 블록 `rig-template-lint` + baseline diff + migrator 체인 pointer. doc-only, 코드 변경 0, 골든 영향 0. **문서·색인 축 6 연속 소진 선언** (120 옵션 → 121 메타 → 122 golden → 123 schema → 124 lint → 125 rig-templates). |
| 자율 후보 완전 소진 공식 기록 | ✅ 완료 (세션 126) | "소진 선언" minimal 세션. PLAN §7 재작성 — 외부 입력 대기 후보 6 축 명시(ADR 0007 Decision / BL-STAGING / BL-VENDOR-KEY / BL-DEPRECATION-POLICY / Runtime 승인 / migrator v1.4.0 리그 변경 합의) + 세션 127 자율 진입 조건 정의(외부 블로커 해제 여부 확인 → 해제 시 해당 후보 진입, 없으면 "소진 재확인" 초단 세션 + ScheduleWakeup 3600s 상한). 저장소 상태 재검토 결과 내부 진입 가능 후보 0 공식 기록 — 내부 문서 축(20+ README / runbook 3 / schema / rig-templates / ADR 7 / notes 1 / sessions 125) + 내부 코드 축(lint C1~C14 포화 · migrator 3 체인 · 렌더러 계약 + 2 구현체 · web-editor wire-through) + 내부 검증 축(golden 30 step · bullmq-integration · validate-schemas 244 · perf SLO · gitleaks) 모두 green. INDEX §4 "세션 127 후보" 로 진입 조건 재정의. doc-only, 코드 변경 0, 골든 영향 0. |
